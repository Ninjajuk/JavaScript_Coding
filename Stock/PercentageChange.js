function calculateChangeArray(oldValues, newValues) {
    ...
    for (let i = 0; i < oldValues.length; i++) {
        ...
        const percentageChange = (oldValue === 0) ? "N/A" : ((absoluteChange / Math.round(oldValue)) * 100).toFixed(2) + "%";
...
// Example data
const oldValues = [342.80, 120, 130, 110, 150];  // initial values
const newValues = [401.75, 130, 110, 150, 180];  // final values
...
for (let i = 0; i < changes.length; i++) {
    ...
    console.log(`  Return: ${changes[i].percentageChange}`);
    console.log();
}
